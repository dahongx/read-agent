...
async def _ppt_task(session_id: str, pdf_path: str, config: PptConfig) -> str:
    ...
    if cached_outputs and cached_outputs.get("ppt_path"):
        session_store.update_path_fields(
            ...
            merged_markdown_path=cached_outputs.get("merged_markdown_path") or None,
...
async def _multi_ppt_task(session_id: str, pdf_paths: list[str], config: PptConfig) -> str:
    ...
    if not pptx_files:
        pptx_files = list(project_dir.glob("*.pptx"))
    logger.info("[MULTI_PPT_TASK][%s] pptx files found: %s", session_id, pptx_files)
    if not pptx_files:
        raise RuntimeError("No PPTX file found after multi-survey generation")
...
async def _multi_rag_task(session_id: str, pdf_paths: list[str]) -> str:
    ...
    await _broadcast_progress(session_id, "rag", "检查联合索引缓存", 5, stage="cache_check")
    await _log(recorder, source="rag", level="INFO", stage="cache_check", message="检查多文档 RAG 缓存")

    h = hashlib.sha256()
    for source_doc in sorted(session.source_documents, key=lambda doc: doc.order):
        h.update(f"{source_doc.order}:".encode("utf-8"))
        h.update(source_doc.content_hash.encode("utf-8"))
    cache_key = f"{h.hexdigest()[:16]}-{cache_version}"

    cache_dir = get_rag_cache_dir(cache_key)
    index_dir = str(cache_dir)
    logger.info("[MULTI_RAG_TASK][%s] cache_key=%s  cache_dir=%s  pdf_count=%d", session_id, cache_key, cache_dir, len(pdf_paths))

    if cache_dir.exists() and (cache_dir / "docstore.json").exists():
        logger.info("[MULTI_RAG_TASK][%s] CACHE HIT → %s", session_id, index_dir)
        session_store.update_path_fields(session_id, rag_index_path=index_dir)
        await _log(recorder, source="rag", level="INFO", stage="cache_check", message="多文档 RAG 缓存命中", details={"index_dir": index_dir, "source_count": len(session.source_documents)})
        await _broadcast_progress(session_id, "rag", "使用已有联合索引", 100, stage="complete", status="completed")
        await _broadcast_session_snapshot(session_id)
        return index_dir
    ...
    source_docs = [doc.model_copy() for doc in sorted(session.source_documents, key=lambda doc: doc.order)]
    rag_future = loop.run_in_executor(None, lambda: build_multi_index(source_docs, index_dir))
    ...
    try:
        while True:
            ...
            if rag_future in done:
                rag_future.result()
                logger.info("[MULTI_RAG_TASK][%s] Index built successfully → %s", session_id, index_dir)
                break
            elapsed = int(loop.time() - started_at)
            await _log(
                recorder,
                ...
                level="INFO",
                stage="embedding",
                message="多文档 RAG 仍在构建中",
                details={"elapsed_seconds": elapsed},
            )
            await _broadcast_progress(
                session_id,
                "rag",
                f"联合向量化中，已运行 {elapsed}s...",
                70,
                stage="embedding",
            )
            if elapsed >= timeout_seconds:
                err = TimeoutError(f"Multi-doc RAG build timed out after {timeout_seconds} seconds")
                setattr(err, "source", "rag")
                setattr(err, "stage", "embedding")
                raise err
    except Exception as e:
        ...
        if not getattr(e, "stage", None):
            setattr(e, "stage", "embedding")
        logger.exception("[MULTI_RAG_TASK][%s] build_multi_index FAILED: %s", session_id, e)
        await _log(recorder, source="rag", level="ERROR", stage="embedding", message="多文档 RAG 构建失败", details={"error": str(e)})
        raise

    session_store.update_path_fields(session_id, rag_index_path=index_dir)
    await _broadcast_progress(session_id, "rag", "联合索引构建完成", 100, stage="complete", status="completed")
    await _log(recorder, source="rag", level="INFO", stage="complete", message="联合索引构建完成", details={"index_dir": index_dir, "source_count": len(source_docs)})
    await _broadcast_session_snapshot(session_id)
    return index_dir
...
async def run_multi_tasks(session_id: str, pdf_paths: list[str], config: PptConfig | None = None) -> None:
    ...
    logger.info(
        "[TASK][%s] run_multi_tasks START  pdf_count=%d  config=%s",
        session_id, len(pdf_paths), config.model_dump(),
    )
    recorder = SessionLogRecorder(session_id)
    ...
    try:
        ...
        ppt_coro = _multi_ppt_task(session_id, pdf_paths, config)
        rag_coro = _multi_rag_task(session_id, pdf_paths)
        ppt_path, rag_index_path = await asyncio.gather(ppt_coro, rag_coro)

        logger.info(
            "[TASK][%s] Multi tasks finished  ppt_path=%s  rag=%s",
        ...
        logger.exception("[TASK][%s] Multi task FAILED: %s", session_id, exc)
        detail = SessionError(
            message=str(exc),
